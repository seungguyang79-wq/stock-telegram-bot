...
app = Flask(__name__)
@app.route('/')
def home(): return "David-Catalyst Stock Bot is Online! ✅"
...
# [핵심] 한글 이름 -> 티커 변환 사전 (종목을 여기에 추가하세요)
TICKER_DICT = {
...
def get_data(symbol):
    try:
        ...
        h = ticker.history(period="5d") # 속도를 위해 5일치만 로드
...
def show_help(cid):
    msg = (
        "🤖 <b>David-Catalyst 명령어 가이드</b>\n\n"
        "• <b>리포트</b> : 주요 지수 및 자산 요약\n"
        "• <b>포트</b> : 내 자산 실시간 수익률\n"
        "• <b>/등록 종목 평단 수량</b> : 자산 추가\n"
        "• <b>/삭제 종목명</b> : 자산 제거\n"
        "• <b>/s 종목명</b> : 실시간 개별 종목 검색\n"
        "• <b>도움말</b> : 이 안내문 보기"
    )
    send_msg(msg, cid)

def run_full_report(cid):
    send_msg("📊 <b>전체 마켓 리포트 생성 중...</b>", cid)
...
def run_portfolio_report(cid):
    pf = load_pf()
    if not pf:
        send_msg("📝 등록된 자산이 없습니다. <code>/등록</code>으로 추가하세요.", cid)
        return
...
if __name__ == "__main__":
    keep_alive()
    MY_PORTFOLIO = load_pf() # 시작 시 저장된 데이터 로드
    while True:
        handle_commands()
        time.sleep(1)
